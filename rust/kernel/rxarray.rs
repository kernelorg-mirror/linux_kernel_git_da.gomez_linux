...
//!
//! This implements an extensible array (aka XArray) via the [`XArray`] type. See [`XArray4`] and
//! [`XArray6`] for common configurations.
//!
//! Users pass and receive [`Entry`], a generic enum with [`Entry::Int`] and [`Entry::Pointer`]
//! variants. The type parameter `P` is the pointer payload: owned (`T`) for store/erase, borrowed
//! (`T::Borrowed<'_>`) for load.
//!
...
//! supported.
//!
//! C `xa_destroy()` frees only internal nodes; callers must free their own stored values. Rust
//! [`Drop`] also frees all stored [`Entry::Pointer`] entries via
//! `ForeignOwnable::from_foreign()`. [`Entry::Int`] values are encoded integers with no backing
//! allocation.
//!
...
//! # C FFI boundary
//!
//! The Rust API enforces entry invariants at compile or construction time. A C FFI layer must
...
//!   `build_assert!(T::FOREIGN_ALIGN >= 4)` only covers Rust callers.
...
/// An entry is either an [`Entry::Int`] integer (`0..=usize::MAX >> 1`) or a 4-byte aligned
/// [`Entry::Pointer`].
///
...
/// # Invariants
///
/// - `Entry::Int(v)`: `v` fits in `usize::BITS - 1` bits (enforced by [`Value`]).
/// - `Entry::Pointer(p)` requires `p` to originate from a type whose [`ForeignOwnable`]
///   implementation returns 4-byte aligned pointers (i.e., `T::FOREIGN_ALIGN >= 4`).
pub enum Entry<P> {
    /// Integer value (`0..=usize::MAX >> 1`).
    Int(Value),
    /// Pointer payload. Owned (`T`) or borrowed (`T::Borrowed<'_>`).
    Pointer(P),
}
...
/// use kernel::rxarray::{Entry, XArray6};
///
...
/// match xa.load(1) {
///     Some(Entry::Pointer(val)) => assert_eq!(*val, 0xbeef_u64),
...
///   entry encoding (enforced at compile time by `Slot::encode()`).
...
impl<T: ForeignOwnable, const SHIFT: usize, const SIZE: usize> XArray<T, SHIFT, SIZE> {
    fn validate() {
        build_assert!(SHIFT > 0, "SHIFT must be > 0");
        build_assert!(SIZE == (1 << SHIFT), "SIZE != 1 << SHIFT");
    }
    ...
    /// as [`Value`].
    pub fn load(&self, index: usize) -> Option<Entry<T::Borrowed<'_>>> {
...
///
/// `PhantomData<T>` gives [`Slot<T>`] the same auto-trait bounds as `T`. [`XArray`] provides
/// explicit `unsafe impl Send/Sync` gated on `T: Send`/`T: Sync`.
///
/// # Invariants
///
...
///   by [`ForeignOwnable::into_foreign()`]. The slot owns the foreign allocation and will free
///   it via [`ForeignOwnable::from_foreign()`] on drop. Bits `1:0 == 0b00` is guaranteed by
///   `T::FOREIGN_ALIGN >= 4` (enforced at compile time in [`Slot::encode()`]).
...
impl<T: ForeignOwnable> Slot<T> {
    ...
    fn encode(entry: Entry<T>) -> Result<Self> {
        ...
        match entry {
            ...
            Entry::Pointer(p) => {
                ...
                // INVARIANT: `bits` is non-null and 4-byte aligned (`bits 1:0 == 0b00`),
                // guaranteed by `build_assert!(T::FOREIGN_ALIGN >= 4)`.
    ...
    fn decode(self) -> Option<Entry<T>> {
        let slot_type = self.is_type();
        let bits = mem::ManuallyDrop::new(self).0;

        match slot_type {
            SlotType::Empty => None,
            // INVARIANT: `bits` is odd, so `bits >> 1` fits in `usize::BITS - 1` bits.
            ...
            SlotType::Pointer => {
                // SAFETY: `is_type()` returned `Pointer`: a valid `T::into_foreign()` pointer.
                // `ManuallyDrop` prevents double-free via `Slot::drop`.
    ...
    fn borrow(&self) -> Option<Entry<T::Borrowed<'_>>> {
        let slot_type = self.is_type();
        let bits = self.0;
        ...
        match slot_type {
            ...
            SlotType::Pointer => {
                // SAFETY: `is_type()` returned `Pointer`: a valid `T::into_foreign()` pointer.
                // `&self` guarantees the pointer remains live for the borrow.
                Some(Entry::Pointer(unsafe { T::borrow(bits as *mut c_void) }))
            }
        }
    }
    ...
    // they reach Rust, where is_type() would misclassify them as Node, causing as_node_ref() to
    ...
    fn is_empty(&self) -> bool {
        matches!(self.is_type(), SlotType::Empty)
    }

    #[inline]
    fn is_node(&self) -> bool {
        matches!(self.is_type(), SlotType::Node)
    }
...
impl<T: ForeignOwnable> Drop for Slot<T> {
    fn drop(&mut self) {
        match self.is_type() {
            SlotType::Pointer => {
                // SAFETY: `is_type()` returned `Pointer`: a valid `T::into_foreign()` pointer.
                // The slot is being dropped, so this is the sole `from_foreign` call.
...
// SAFETY: `XArray<T>` owns `T` values (via `ForeignOwnable::into_foreign` pointers encoded as
// `usize`). Transferring the array across threads transfers ownership of all contained values. This
// is safe when `T: Send`, the same condition as for a struct with a field of type `T`.
unsafe impl<T: ForeignOwnable + Send, const SHIFT: usize, const SIZE: usize> Send
    for XArray<T, SHIFT, SIZE>
{
}

// SAFETY: Sharing `&XArray<T>` between threads allows concurrent `load()` calls, each of
// which calls `T::borrow()` on the same foreign pointer. This is safe when `T: Sync`, because
// `T::Borrowed<'_>` then supports concurrent creation. Mutation (`store`/`erase`) requires
// `&mut self`, so Rust's borrow checker prevents concurrent read/write access without external
// synchronization.
unsafe impl<T: ForeignOwnable + Sync, const SHIFT: usize, const SIZE: usize> Sync
    for XArray<T, SHIFT, SIZE>
{
}
...
impl<T: ForeignOwnable, const SHIFT: usize, const SIZE: usize> Node<T, SHIFT, SIZE> {
    ...
    fn erase(&mut self, index: usize, level: usize) -> Option<Entry<T>> {
        ...
        if slot.is_node() {
            // SAFETY: `slot.is_node()` is true. By `Node` type invariant, the slot holds a valid
            // `KBox<Node>`. `&mut self` guarantees exclusive access. The borrow of `child` ends
            // before `mem::take` below.
            let child = unsafe { slot.as_node_mut::<SHIFT, SIZE>() };
            let old_value = child.erase(index, level - 1);
            if child.is_empty() {
                let old_slot = mem::take(slot);
                // SAFETY: Node checked above, preserved by `mem::take`. By `Node` type invariant, it
                // holds a valid `KBox<Node>`.
    ...
    fn store(
        ...
        if slot.is_node() {
            // SAFETY: `slot.is_node()` is true. By `Node` type invariant, the slot holds a valid
            // `KBox<Node>`. `&mut self` guarantees exclusive access.
    ...
    fn load(&self, index: usize, level: usize) -> Option<Entry<T::Borrowed<'_>>> {
        let slot_index = Self::slot_index(index, level);
        ...
        if slot.is_node() {
            // SAFETY: `slot.is_node()` is true. By `Node` type invariant, the slot holds a valid
            // `KBox<Node>`. `&self` satisfies aliasing.
...
mod tests {
    ...
    fn overwrite_value_impl<T: ForeignOwnable, const SHIFT: usize, const SIZE: usize>() {
        ...
        match xa.load(137) {
            Some(Entry::Int(v)) => assert_eq!(v, 2002),
            _ => panic!("expected Int"),
        };
    }
    ...
    fn erase_value_impl<T: ForeignOwnable, const SHIFT: usize, const SIZE: usize>() {
        ...
        match xa.load(139) {
            Some(Entry::Int(v)) => assert_eq!(v, 1003),
            _ => panic!("expected Int at neighbor"),
        };
    }
    ...
    fn erase_nonexistent_impl<T: ForeignOwnable, const SHIFT: usize, const SIZE: usize>() {
        ...
        match xa.load(0) {
            Some(Entry::Int(v)) => assert_eq!(v, 1),
    ...
    fn store_load_pointer_impl<const SHIFT: usize, const SIZE: usize>() {
        ...
        match xa.load(0) {
            Some(Entry::Pointer(val)) => assert_eq!(*val, 42u64),
            _ => panic!("expected Pointer"),
        }
    }
    ...
    fn mixed_values_and_pointers_impl<const SHIFT: usize, const SIZE: usize>() {
        ...
        match xa.load(0) {
            Some(Entry::Int(v)) => assert_eq!(v, 100),
            _ => panic!("expected Int"),
        }
        match xa.load(1) {
            Some(Entry::Pointer(val)) => assert_eq!(*val, 42u64),
    ...
    fn value_encoding_boundary_impl<T: ForeignOwnable, const SHIFT: usize, const SIZE: usize>() {
        ...
        // Compile-time: Entry::int validates via build_assert in Bounded::new.
        xa.store(0, Entry::int::<{ usize::MAX >> 1 }>(), flags::GFP_KERNEL)
            .unwrap();
        match xa.load(0) {
            Some(Entry::Int(v)) => assert_eq!(v, max),
    ...
    fn overwrite_pointer_with_pointer_impl<const SHIFT: usize, const SIZE: usize>() {
        ...
        match xa.load(0) {
            Some(Entry::Pointer(val)) => assert_eq!(*val, 99u64),
